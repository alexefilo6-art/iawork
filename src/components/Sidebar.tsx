  ...
  return (
    <div className="w-full md:w-64 border-r border-stone-200 bg-stone-50 flex flex-col h-full relative">
      ...
      {docToDelete && (
        <div className="fixed inset-0 bg-stone-900/20 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white w-full max-w-sm rounded-2xl shadow-2xl p-6 border border-stone-100">
            <h3 className="text-lg font-serif font-bold text-stone-900 mb-2">Delete Document?</h3>
            <p className="text-sm text-stone-500 mb-6">Are you sure you want to delete this document? This action cannot be undone.</p>
            <div className="flex gap-3">
              <button
                onClick={() => setDocToDelete(null)}
                className="flex-1 py-2.5 rounded-xl font-medium bg-stone-100 text-stone-600 hover:bg-stone-200 transition-all"
              >
                Cancel
              </button>
              <button
                onClick={() => deleteDocument(docToDelete)}
                className="flex-1 py-2.5 rounded-xl font-medium bg-red-600 text-white hover:bg-red-700 transition-all shadow-lg shadow-red-600/10"
              >
                Delete
              </button>
            </div>
